...
class POSingAgent1W1F:   
    def __init__(self, *args, **kwargs):  
        ...
        low_act = np.zeros(((5 + 1) * self.product_types_num), dtype=np.float32)
        high_act = np.zeros(((5 + 1) * self.product_types_num), dtype=np.float32)
        high_act[:1] = self.retailer_capacity
        high_act[1:2] = self.price_capacity
        high_act[2:3] = 1.0
        high_act[3:4] = self.retailer_capacity
        high_act[4:5] = self.price_capacity
        high_act[5:6] = self.v_f
        ...
        high_obs = np.zeros((16,), dtype=np.float32)
        high_obs[:factory_size*7] = factory_capacity
        high_obs[factory_size*7:factory_size*7+1] = self.periods
        high_obs[factory_size*8:factory_size*15] = factory_capacity
        high_obs[factory_size*15:factory_size*15+1] = self.periods
        self.observation_space = Box(low=low_obs, high=high_obs, dtype=np.float32,seed=2024)
        #self.actio
        # self.action_space = Box([retailer_capacity + 1,price_capacity+1,self.v_f,retailer_capacity + 1,price_capacity+1,self.v_f])  #[order , price, show]  
        
        # self.observation_space = Box([factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,self.periods+1,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,factory_capacity,self.periods+1]) # [inventory,backlog,stockout,Factory_inventory]  
    ...
    def step(self, action):  
        ...
        reward = self.reward(obs_dict, action[0],action[1],  action[3], action[4], demand,observations_dict)   
        done = self.is_done()  
        ...
        self.render()
        return observations_dict, reward, done,done, {}   
        ...
        if ep_count % p == 0 and len(self.state_history) % p == 0:
            ...
            for i in range(2):
                ...
                if i == 0:
                    ...
                    ax.plot(range(start, end), [s[5] for s in self.state_history[start:end]], label=f'Customer Demand(Real)', color='b', alpha=0.85, linewidth=0.8)
                else:
            ...
            ax.plot(range(start, end), [r for r in self.reward_history[start:end]], label='Profit', color="darkviolet", alpha=0.8, linewidth=0.65)
...
class POSAgent1W1F_V2T1(POSingAgent1W1F, gymnasium.Env):
    def __init__(self, *args, **kwargs):
        ...
        high_act[2] = 0.0
        high_act[3] = self.retailer_capacity
        high_act[4] = self.price_capacity
        high_act[5] = self.v_f
        self.action_space = Box(low=low_act, high=high_act, dtype=np.float32,seed=2024)
